...
public interface AnnonceRepository extends JpaRepository<Annonce, UUID> {
    ...
    @Query("SELECT a FROM Annonce a WHERE " +
            "(:zoneGeographique IS NULL OR (a.latitude IS NOT NULL AND a.longitude IS NOT NULL)) AND " +
            "(:etat IS NULL OR a.etat = :etat) AND " +
            "(:modeDeRemise IS NULL OR a.modeDeRemiseEnum = :modeDeRemise) AND " +
            "(:dateDePublication IS NULL OR a.datePublication >= :dateDePublication) AND " +
            "(:motsCles IS NULL OR EXISTS (" +
            "SELECT 1 FROM Annonce a2 JOIN a2.motsCles m WHERE a2 = a AND m IN :motsCles" +
            "))")
    List<Annonce> searchAnnonces(
            @Param("zoneGeographique") String zoneGeographique,
